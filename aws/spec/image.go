...
type UpdateImage struct {
	...
	Users        []*string `awsName:"UserIds" awsType:"awsstringslice" templateName:"users"`
...
func (cmd *UpdateImage) ValidateParams(params []string) ([]string, error) {
	return paramRule{
		tree: allOf(node("id"), oneOf(allOf(node("operation"), oneOf(node("groups"), node("users"))), node("product-codes"), node("description"))),
	}.verify(params)
}
...
func (cmd *UpdateImage) prepareImageAttributeInput(ctx map[string]interface{}) (*ec2.ModifyImageAttributeInput, error) {
	...
	if cmd.Users != nil || cmd.Groups != nil {
